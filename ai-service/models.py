...
# ---------------------------------------------------------------------------
# Embedding extraction models (Phase 5 — pgvector path)
# ---------------------------------------------------------------------------


class ExtractEmbeddingsRequest(BaseModel):
    """
    Request payload for POST /extract-embeddings.

    Accepts a Base64-encoded classroom photo. The service detects all faces
    and returns their 128-D embeddings without performing any student matching.
    Matching is done in PostgreSQL via pgvector (<=> cosine distance operator)
    so the payload never carries student data.
    """
...
class ExtractEmbeddingsResponse(BaseModel):
    """
    Response payload from POST /extract-embeddings.

    face_count:  Number of faces detected in the classroom image.
    embeddings:  List of 128-D float vectors, one per detected face.
                 Each vector is L2-normalised by SFace before being returned.
    """
